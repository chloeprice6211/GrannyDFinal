...
public class NL_SRS_SnowCoverage_GUI : SRS_CoverageShaderGUI_base
{
    ...
    public override void ShaderPropertiesGUI(Material material)
    {
        ...
        if (coverageProperties)
        {
            ...
            {
                EditorGUILayout.HelpBox("There's no Snow Coverage instance in the scene. Please add one to make the snow shaders work correctly.", UnityEditor.MessageType.Warning);
            ...
            if(coverage.prop.floatValue == 1)//coverage switch
            {
                ...
                if (paintableCoverage.prop.floatValue == 1 && m_MaterialEditor.IsInstancingEnabled()) 
                    EditorGUILayout.HelpBox("GPU Instancing is enabled on this material, this will break vertex colors on different objects. Consider disable instancing if you want to use vertex colors.", UnityEditor.MessageType.Warning);
