...
public class NL_SRS_RainCoverage_GUI : SRS_CoverageShaderGUI_base
{
    ...
    public override void ShaderPropertiesGUI(Material material)
    {
        ...
        {
            if (RainCoverage.instance == null) EditorGUILayout.HelpBox("There's no Rain Coverage instance in the scene. Please add one to make the rain shaders work correctly.", UnityEditor.MessageType.Warning);
            else
            {
                GUILayout.Space(5);
            ...
            if (coverage.prop.floatValue == 1)//coverage switch
            {
                ...
                if (material.GetFloat("_PaintableCoverage") == 1 && m_MaterialEditor.IsInstancingEnabled())
                    EditorGUILayout.HelpBox("GPU Instancing is enabled on this material, this will break vertex colors " +
                        "on different objects. Consider disable instancing if you want to use vertex colors.", UnityEditor.MessageType.Warning);
